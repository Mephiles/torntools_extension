...
function displayWarning() {
	if (userdata.energy.current > userdata.energy.maximum && userdata.chain.current > 10 && settings.pages.attack.disable_attack_stacked_warning_until - new Date() <= 0) {
		...
		doc.find("button.tt-silver-button.tt-dnd-button").addEventListener("click", (event) => {
			...
			settings.disable_attack_stacked_warning_until = new Date(new Date().getTime() + 30*60*1000);
		});
	}
}

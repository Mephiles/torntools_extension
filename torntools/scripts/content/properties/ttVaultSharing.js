window.addEventListener("load", async function(event) {
    ...
    local_storage.get(["settings", "vault"], function([settings, vault]){
        if(!settings.pages.properties.vault_sharing){
            return;
        }

        if(window.location.hash != "#/p=options&tab=vault"){
    ...
    async function Main(vault, need_to_save){
        console.log("VAULT", vault);
        let vault_container = content.new_container("TornTools - Vault Sharing", {id: "tt-vault-container", next_element: doc.find(".vault-wrap+hr")}).find(".content");
...
//         let page_break = doc.find(".vault-wrap+hr");
//         let vault_container = content.new_container("TornTools - Vault Sharing", {id: "tt-vault-container", next_element: page_break}).find(".content");
...
//                     if(user){
//                         vault.user.current_money = type == "withdraw" ? vault.user.current_money - amount : vault.user.current_money + amount;
//                         local_storage.change({"vault": {"user": {"current_money": vault.user.current_money}}});
//                     } else {
//                         vault.partner.current_money = type == "withdraw" ? vault.partner.current_money - amount : vault.partner.current_money + amount;
//                         local_storage.change({"vault": {"partner": {"current_money": vault.user.current_money}}});
//                     }
...
//         left_input.addEventListener("keyup", function(){
//             let left_value = parseInt(left_input.value.replace(/,/g, "")) || 0;
//             let right_value = parseInt(total_money) - left_value;
...
//         right_input.addEventListener("keyup", function(){
//             let right_value = parseInt(right_input.value.replace(/,/g, "")) || 0;
//             let left_value = parseInt(total_money) - right_value;
...
//             console.log("User", user_value);
//             console.log("Partner", partner_value);
...
//             let last_transaction = new Date(`${months[date_parts[1]-1]} ${date_parts[2]}, ${date_parts[0]} ${time}`);
//             console.log("Last Transaction", last_transaction);
...
//             if(!event.target.parentElement.parentElement.classList.contains("disable")){
//                 let amount = parseInt(doc.find(".withdraw-icon+div input[name=withdraw]").getAttribute("value"));
//                 local_storage.change({"vault": {"user": {"current_money": vault.user.current_money - amount}}});
//                 containerRemoved().then(function(){
//                     Main(settings, vault);
//                 });
//             }
...
//                 console.log()
//                 let amount = parseInt(doc.find(".deposit-icon+div input[name=deposit]").getAttribute("value"));
//                 local_storage.change({"vault": 
//                 {"user": {"current_money": vault.user.current_money + amount}},
//                 "last_transaction": __
//                 });
//                 containerRemoved().then(function(){
//                     Main(settings, vault);

...
function addTableContent(travel_items) {
	...
	if (userdata && userdata.status && userdata.status.state === "Traveling" && userdata.travel) {
		let country = (userdata.travel.destination == "Torn" ? userdata.status.description.match(/Returning to Torn from (.*)/i)[1].toLowerCase() : userdata.travel.destination).toLowerCase();

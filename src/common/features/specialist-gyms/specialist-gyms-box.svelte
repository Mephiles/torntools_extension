<script lang="ts">
	...
	let { stats, selectedSpecialGym1 = $bindable(), selectedSpecialGym2 = $bindable(), gymsData }: SpecialistGymsBoxProps = $props();
		...
		<div class="content">
			<div class="selects">
				<select bind:value={() => selectedSpecialGym1, specialGymSelected1}>
					{#each specialGymOptions as specialGymOption}
						<option value={specialGymOption.value}>{specialGymOption.description}</option>
					{/each}
				</select>
			...
			<div class="info">
				...
				{:else if gymsData.type === "impossible"}
					<div class="gyms-message">This combination of specialist gyms is impossible.</div>
				{:else}
					<div class="stats-info">
						{#each battleStats as battleStat}
							{@const statValue = stats[battleStat] + gymsData.missing[battleStat]}
							<div class="stat-info">

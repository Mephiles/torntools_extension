...
import { createSelect } from "@utils/elements/select/select";
...
import { observeChain } from "@utils/functions/requires";
import { getPageStatus } from "@utils/functions/torn";
import { toRecord } from "@utils/functions/utilities";
...
function calculateSpecialGymsData(
	...
	const twoStatsConfig =
		selectionOneSpecialGymInfo.type === SPECIAL_GYM_TYPE.SINGLE_STAT ? (selectionTwoSpecialGymInfo as TwoStatsSpecialGymInfo) : selectionOneSpecialGymInfo;
...
function createSpecialistGymsBoxElement(prevElement: Element, getStatsFn: () => Record<BATTLE_STAT, number>, statsChangeFn: () => void) {
	...
	specialGymSelectOne.onChange(async () => {
	...
	specialGymSelectTwo.onChange(async () => {

...
import { createFilterEnabledFunnel, createFilterSection, createStatistics } from "@utils/functions/filters";
...
import { getPageStatus, RANK_TRIGGERS } from "@utils/functions/torn";
...
async function initialiseFilters() {
	...
	const listObserver = new MutationObserver((mutations) => {
		if (mutations.some((mutation) => Array.from(mutation.addedNodes).some((node) => isElement(node) && node.matches("li[class*='tableRow__']")))) {
			if (filterSetupComplete && FEATURE_MANAGER.isEnabled(EnemyFilterFeature)) applyFilters();
		}
	});

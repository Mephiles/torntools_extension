...
export class ScouterService {
	...
	toCache(result: ScouterResult) {
		ttCache.set({ [result.player_id]: result }, "fair_fight" in result && result.fair_fight ? TO_MILLIS.DAYS : TO_MILLIS.MINUTES * 5, this.cacheKey);
	}
...
export function scouterService() {
 ...
 */
export function ffColor(value: number) {
 ...
 * Credits to rDacted [2670953] (https://www.torn.com/profiles.php?XID=2670953).
 */
export function contrastFFColor(hex: string) {
	// Convert hex to RGB

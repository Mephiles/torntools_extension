...
import { torndata, userdata } from "@utils/data/database";
import { hasAPIData } from "@utils/functions/api";
import { requireCondition, requireElement } from "@utils/functions/requires";
import { getCookie, isIntNumber, TO_MILLIS } from "@utils/functions/utilities";
...
export const NON_ATTACKABLE_ACCOUNTS: { id: number; reason: "Admin" | "NPC" }[] = [
	{ id: 1, reason: "Admin" },
	{ id: 4, reason: "NPC" },
	{ id: 7, reason: "NPC" },
	{ id: 9, reason: "NPC" },
...
export const COMPANY_INFORMATION: { [name: string]: CompanyInformation } = {
	"Adult Novelties": {
		1: {
			name: "Blackmail",
			cost: "1",
...
export const ARMOR_SETS = ["Assault", "Riot", "Dune", "Delta", "Marauder", "EOD", "Sentinel", "Vanguard"] as const;

export function getNextChainBonus(current: number) {
...
type CountryOption = "south-africa";

export function isInCountry(country: CountryOption) {
	return isAbroad() && document.body.dataset.country === country;
}
...
export function hasDarkMode() {
	return RUNTIME_ENVIRONMENT.isExtensionPage() ? document.body.classList.contains("dark") : document.body.classList.contains("dark-mode");
}
...
export function createMessageBox(content: string, partialOptions: Partial<MessageBoxOptions> = {}) {
	...
	return elementBuilder({
		...
		children: [
			elementBuilder({
				...
				children: [elementBuilder({ type: "div", class: "tt-message-icon", children: [torntools()] })],
			}),
			elementBuilder({
				type: "div",
...
export enum REACT_UPDATE_VERSIONS {
	DEFAULT = "default",
...
export function updateReactInput(input: HTMLInputElement | HTMLTextAreaElement, value: string | number, partialOptions: Partial<ReactInputOptions> = {}) {
...
export function isDividendStock(id: string | number) {
	let _id: number;
...
export function millisToNewDay() {
	const now = Date.now();
...
export async function createTTTopLinks() {
	if (ttTopLinks) {
...
export function getHospitalizationReason(details: string) {

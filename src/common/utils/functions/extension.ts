import { BADGE_MANAGER } from "@utils/context";
import { SCRIPT_TYPE } from "@utils/functions/utilities";
...
export async function setBadge(type: keyof typeof BADGE_TYPES, partialOptions: Partial<BadgeOptions> = {}): Promise<boolean> {
	...
	void BADGE_MANAGER.setBadgeText(badge.text || "");
	if (badge.color) void BADGE_MANAGER.setBadgeBackgroundColor(badge.color);
	return true;
}

export function getBadgeText(): Promise<string | null> {
	if (SCRIPT_TYPE !== "BACKGROUND") return Promise.resolve(null);

	return BADGE_MANAGER.getBadgeText();
}

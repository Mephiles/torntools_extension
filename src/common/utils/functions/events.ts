...
export enum EVENT_CHANNELS {
	...
	RACING__CUSTOM_RACES__LIST = "RACING__CUSTOM_RACES__LIST",
	RACING__SELECT_CAR_CUSTOM = "RACING__SELECT_CAR_CUSTOM",
	RACING__SELECT_CAR_CUSTOM_CREATED = "RACING__SELECT_CAR_CUSTOM_CREATED",
	RACING__CHANGE_CAR = "RACING__CHANGE_CAR",
}
...
export interface EventPayloads {
	...
	[EVENT_CHANNELS.CRIMES2_CRIME_LOADED]: {
		crime: (typeof CRIMES2)[keyof typeof CRIMES2];
		crimeRoot: Element;
		page: string;
		url: string;
		json: any;
	...
	[EVENT_CHANNELS.RACING__CUSTOM_RACES__LIST]: never;
	[EVENT_CHANNELS.RACING__SELECT_CAR_CUSTOM]: { id: number };
	[EVENT_CHANNELS.RACING__SELECT_CAR_CUSTOM_CREATED]: { trackId: number };
	[EVENT_CHANNELS.RACING__CHANGE_CAR]: never;
}
...
export function triggerCustomListener<T extends keyof EventPayloads>(channel: T, payload?: EventPayloads[T]): void {
	if (ANTI_SCRAPE_EVENTS.includes(channel) && !isTabFocused()) return;

	EVENT_HANDLER.triggerEvent(channel, payload);
}

export function addCustomListener<T extends keyof EventPayloads>(channel: T, listener: CustomEventListener<T>): void {

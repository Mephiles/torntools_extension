import { RUNTIME_ENVIRONMENT, ttStorage } from "@utils/context";
import { api, factiondata, settings, userdata } from "@utils/data/database";

import { ttUsage } from "@utils/data/usage";
import { getBadgeText, setBadge } from "@utils/functions/extension";
import { getRFC } from "@utils/functions/torn";
import { SCRIPT_TYPE, TO_MILLIS } from "@utils/functions/utilities";
...
export type FetchLocation = keyof typeof FETCH_PLATFORMS | typeof FETCH_PLATFORMS.tornstats;

export async function fetchData<R = any>(l: FetchLocation, partialOptions: Partial<FetchOptions> = {}): Promise<R> {
...
async function hasOrigins(...origins: string[]): Promise<boolean> {
	return RUNTIME_ENVIRONMENT.hasPermission(origins);
}

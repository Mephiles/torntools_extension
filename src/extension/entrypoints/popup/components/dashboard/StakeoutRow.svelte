...
<div class="bg-card rounded-lg border p-2 text-xs">
	<div class="flex items-start gap-2">
		<a
			class="hover:bg-muted/60 -m-1 block min-w-0 flex-1 rounded-md p-1"
		...
		>
			<div class="flex items-center gap-1.5">
				...
				{#if row.label}<span class="text-foreground/65 truncate">({row.label})</span>{/if}
			</div>
			<div class="text-foreground/80 mt-1 grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2 text-[10px]">
				<span>Life</span>
				<div class="bg-muted h-1.5 flex-1 overflow-hidden rounded-sm">
			...
			<div class="mt-1 flex items-center justify-between gap-2">
				...
				<span class="text-foreground/80 shrink-0">Last action: {row.lastAction}</span>
			</div>
		</a>

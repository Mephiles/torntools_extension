<script lang="ts">
	...
	import type { BooleanPreferenceStoragePath, NumberPreferenceStoragePath } from "@/entrypoints/options/components/preferences/preference-storage";
...
<Dialog.Root bind:open>
	...
	<Dialog.Content class="sm:max-w-sm md:max-w-md">
		<Dialog.Header>
			...
			<Dialog.Description>
				Configure your service preferences for {title.toLowerCase()} here. Lowest priority goes first. Services with the same priority will run at the same
				time and will compete for the {usesBest ? "best" : "fastest"} response.
			</Dialog.Description>
		</Dialog.Header>

		<!-- Table headers -->
		<div class="space-y-1">
			<div class="text-muted-foreground grid grid-cols-[1fr_auto_auto] items-center gap-1 text-sm">
				<span>Service</span>
				<span class="w-18 text-center">Enabled</span>
				<span class="w-18 text-center">Priority</span>
			</div>

			{#each services as service (service.name)}
				<div class="border-border bg-background/60 grid grid-cols-[1fr_auto_auto] items-center gap-1 rounded-md border p-2">
					<span class="text-sm">{service.name}</span>
					<div class="flex w-18 justify-center">
						<Switch size="sm" checked={isEnabled(service)} onCheckedChange={(checked) => setEnabled(service, checked)} />
					</div>
					<Input
						type="number"
						min={1}

...
import { applyPlural, capitalizeText, dropDecimals, formatNumber, formatTime } from "@common/utils/functions/formatting";
import { getHospitalizationReason, getNextChainBonus, hasFinishedEducation, LINKS, MAX_MISSIONS } from "@common/utils/functions/torn";
import { getUTCTodayAtTime, hasTimePassed, isSameUTCDay, TO_MILLIS } from "@common/utils/functions/utilities";
...
export async function updateUserdata(forceUpdate = false) {
	...
	if (updateEssential) {
		...
		for (const selection of ["cooldowns", "icons", "newmessages", "money", "travel", "refills", "bars"]) {

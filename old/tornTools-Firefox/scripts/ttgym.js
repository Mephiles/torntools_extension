...
function Main(){
	chrome.storage.local.get(["settings", "userdata", "torndata"], function(data){
		...
		if(ach){
			...
			var items = {
				"Str": {stats: cleanNr(ps["strength"]), ach: [], alt: ["Strength"], incl: ["gain"]},
				"Def": {stats: cleanNr(ps["defense"]), ach: [], alt: ["Defense"], incl: ["gain"]},
				"Spe": {stats: cleanNr(ps["speed"]), ach: [], alt: ["Speed"], incl: ["gain"]},
				"Dex": {stats: cleanNr(ps["dexterity"]), ach: [], alt: ["Dexterity"], incl: ["gain"]},
				"Total": {stats: cleanNr(ps["total"]), ach: [], incl: ["stats"]}
			}

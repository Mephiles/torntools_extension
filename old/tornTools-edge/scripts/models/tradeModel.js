...
export async function calculateItemsValue(items, api_key, pricelist){
	if(items === "none"){
	...
	for(let id in items){
		if(pricelist && pricelist[id]){
			...
			value += await this.get_api(`https://api.torn.com/market/${id}?selections=bazaar,itemmarket`, api_key)
			.then(data => {

...
async function fetchData<T = any>(location: any, options: any = {}): Promise<T> {
	options = {
		fakeResponse: false,
		section: undefined,
		id: undefined,
	...
	return new Promise(async (resolve, reject) => {
		if (options.relay && SCRIPT_TYPE !== "BACKGROUND") {
			// @ts-ignore Types only include promise version, while we still use callback (for now).
...
function changeAPIKey(key): Promise<void> {
	return new Promise((resolve, reject) => {

		...
		for (const id in settings.notifications.types.stocks) {
			...
			if (alerts.priceFalls && oldStocks[id].current_price > alerts.priceFalls && stocks[id].current_price <= alerts.priceFalls) {
				await notifyUser(
					...
					`(${stocks[id].acronym}) ${stocks[id].name} has fallen to ${formatNumber(stocks[id].current_price, { currency: true })} (alert: ${formatNumber(
						alerts.priceFalls,
						{ currency: true }
					)})!`,
					LINKS.stocks
				...
				await notifyUser(
					"TornTools - Stock Alerts",
					`(${stocks[id].acronym}) ${stocks[id].name} has reached ${formatNumber(stocks[id].current_price, { currency: true })} (alert: ${formatNumber(
						alerts.priceReaches,
						{ currency: true }
					)})!`,
					LINKS.stocks

...
async function setupPreferences(requireCleanup: boolean = false) {
	...
	function requestOrigin(origin: string, event: Event) {
		const target = event.target as HTMLInputElement;
	...
	function requestPermissions() {
		...
		browser.permissions.contains({ origins }, (granted) => {
			if (granted) return;
			...
			loadConfirmationPopup({
				...
				.then(() => {
					browser.permissions.request({ origins }, (granted) => {
						if (granted) return;
...
async function setupAPIInfo() {
	...
	document.querySelector(".current-usage .yata .icon").innerHTML = await (await fetch(browser.runtime.getURL("/images/svg-icons/yata.svg"))).text();
	...
	const canvas = document.querySelector<HTMLCanvasElement>("#current-usage-chart");
	...
	const usageChart = new Chart(canvas, {
		...
		data: {
			labels: [],
			datasets: [
				{ label: "Torn", data: [], backgroundColor: "#666" },
				{ label: "Tornstats", data: [], backgroundColor: "#8abeef" },
				{ label: "YATA", data: [], backgroundColor: "#447e9b" },
			],
		},
		options: {
			animation: {
	...
	function updateUsage(usageChart: Chart<"bar">, position: string) {
		...
		usageChart.data.datasets[0].barThickness = barThickness;
...
async function setupExport() {
	...
	async function getExportData(api: boolean): Promise<ExportData> {
		...
		const data: ExportData = {
			...
			database: (await ttStorage.get(exportedKeys))
				.filter((x) => !isNumber(x))
				.reduce((object, value, index) => {
					if (!isNumber(value)) object[exportedKeys[index]] = value;
					return object;
	...
	function loadSync() {
		...
		new Promise<ExportData | ExportError>((resolve) => {
			browser.storage.sync.get(null, (data) => {
				if (Object.keys(data).length && "database" in data) resolve(data as unknown as ExportData);

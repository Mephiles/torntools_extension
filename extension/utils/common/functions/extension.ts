import { SCRIPT_TYPE } from "@/utils/common/functions/utilities";
...
export async function setBadge(type: keyof typeof BADGE_TYPES, partialOptions: Partial<BadgeOptions> = {}): Promise<boolean> {
	...
	void browser.action.setBadgeText({ text: badge.text || "" });
	if (badge.color) void browser.action.setBadgeBackgroundColor({ color: badge.color });
	return true;
}
